...
def call_llm_api(pdf_path):
    ...
    llm_api_url = "https://anvichip--pixiplot-cli.modal.run"  # URL of the LLM API
    ...
    # Define payload for the first task
        payload = {'auth': 1, 'text': processed_text, 'task_num': 1}
        ...
        print("Task 3")
        print(response.text)
        final_prompts.append(response.text)
        final_panels.append(response_panel.text)
        
    
    return final_prompts, final_panels
